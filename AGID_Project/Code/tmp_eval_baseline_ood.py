"""Quick OOD eval for BaselinePlain checkpoint (no concept bottleneck).

Mirrors cbnet_agid.eval_ood but loads BaselinePlain instead of CBNetAGID.
Reuses the same sampling/eval logic for comparability.
"""
...
def val_samples(root, gen_dir, n_per_class=None, seed=0, real_dir=None):
    val_dir = root / gen_dir / "val"
    ai_dir = val_dir / "ai"
    ...
    print(f"  {gen_dir:<35s}  val ai={len(ai)}  nature={len(real)}")
    return [(p, 1) for p in ai] + [(p, 0) for p in real]


def evaluate(model, samples, transform, batch_size, num_workers, device, name):
...
def main():
    ...
    print(f"\n[STEP 1] loading BaselinePlain checkpoint: {args.ckpt}")
    ckpt = torch.load(args.ckpt, map_location=device, weights_only=False)
    model = BaselinePlain(pretrained=False, signal_channels=512).to(device)
    model.load_state_dict(ckpt["model"])
    print(f"  loaded epoch {ckpt['epoch']}  ({sum(p.numel() for p in model.parameters())/1e6:.1f}M params)")
    ...
    print(f"\n{'='*60}")
    print("OOD SUMMARY (no-bottleneck BaselinePlain)")
    print(f"{'='*60}")
